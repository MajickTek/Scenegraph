...
public class SWBlend_SRC_OUTPeer extends SWEffectPeer {
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float blend_src_out_res_x = top_x * (1.0F - bot_w);
            float blend_src_out_res_y = top_y * (1.0F - bot_w);
            float blend_src_out_res_z = top_z * (1.0F - bot_w);
            float blend_src_out_res_w = top_w * (1.0F - bot_w);
            ...
            if (blend_src_out_res_x < 0.0F) {
               color_x = 0.0F;
            } else if (blend_src_out_res_x > 1.0F) {
               color_x = 1.0F;
            }

            if (blend_src_out_res_y < 0.0F) {
               color_y = 0.0F;
            } else if (blend_src_out_res_y > 1.0F) {
               color_y = 1.0F;
            }

            if (blend_src_out_res_z < 0.0F) {
               color_z = 0.0F;
            } else if (blend_src_out_res_z > 1.0F) {
               color_z = 1.0F;
            }

            if (blend_src_out_res_w < 0.0F) {
               color_w = 0.0F;
            } else if (blend_src_out_res_w > 1.0F) {
               color_w = 1.0F;
            }

...
public class SWPhongLighting_SPOTPeer extends SWEffectPeer {
   ...
   public SWPhongLighting_SPOTPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      float lightSpecularExponent = this.getLightSpecularExponent();
      ...
      float[] normalizedLightDirection_arr = this.getNormalizedLightDirection();
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float dot_res = (float)Math.sqrt((double)(Lxyz_x * Lxyz_x + Lxyz_y * Lxyz_y + Lxyz_z * Lxyz_z));
            normalize_res_z = Lxyz_z / dot_res;
            normalize_res_y = Lxyz_y / dot_res;
            normalize_res_x = Lxyz_x / dot_res;
            Lxyz_x = normalize_res_x;
            Lxyz_y = normalize_res_y;
            Lxyz_z = normalize_res_z;
            dot_res = normalize_res_x * normalizedLightDirection_x + normalize_res_y * normalizedLightDirection_y + normalize_res_z * normalizedLightDirection_z;
            float Lrgb_x = 0.0F;
            float min_res = dot_res < Lrgb_x ? dot_res : Lrgb_x;
            Lrgb_x = -min_res;
            float pow_res = (float)Math.pow((double)Lrgb_x, (double)lightSpecularExponent);
            Lrgb_x = lightColor_x * pow_res;
            ...
            dot_res = N_x * Lxyz_x + N_y * Lxyz_y + N_z * Lxyz_z;
            ...
            pow_res = (float)Math.pow((double)dot_res, (double)specularExponent);

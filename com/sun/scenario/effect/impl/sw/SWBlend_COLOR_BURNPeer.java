...
public class SWBlend_COLOR_BURNPeer extends SWEffectPeer {
   public SWBlend_COLOR_BURNPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float res_x = 1.0F - (1.0F - bot_x) / top_x;
            float res_y = 1.0F - (1.0F - bot_y) / top_y;
            float res_z = 1.0F - (1.0F - bot_z) / top_z;

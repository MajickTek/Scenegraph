...
public class SWBlend_SRC_ATOPPeer extends SWEffectPeer {
   public SWBlend_SRC_ATOPPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float blend_src_atop_res_x = top_x * bot_w + bot_x * (1.0F - top_w);
            float blend_src_atop_res_y = top_y * bot_w + bot_y * (1.0F - top_w);
            float blend_src_atop_res_z = top_z * bot_w + bot_z * (1.0F - top_w);
            float blend_src_atop_res_w = top_w * bot_w + bot_w * (1.0F - top_w);
            ...
            if (blend_src_atop_res_x < 0.0F) {
               color_x = 0.0F;
            } else if (blend_src_atop_res_x > 1.0F) {
               color_x = 1.0F;
            }

            if (blend_src_atop_res_y < 0.0F) {
               color_y = 0.0F;
            } else if (blend_src_atop_res_y > 1.0F) {
               color_y = 1.0F;
            }

            if (blend_src_atop_res_z < 0.0F) {
               color_z = 0.0F;
            } else if (blend_src_atop_res_z > 1.0F) {
               color_z = 1.0F;
            }

            if (blend_src_atop_res_w < 0.0F) {
               color_w = 0.0F;
            } else if (blend_src_atop_res_w > 1.0F) {
               color_w = 1.0F;
            }

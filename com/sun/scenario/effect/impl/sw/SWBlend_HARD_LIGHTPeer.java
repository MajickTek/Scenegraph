...
public class SWBlend_HARD_LIGHTPeer extends SWEffectPeer {
   public SWBlend_HARD_LIGHTPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float lumCoeff_x = 0.2125F;
            float lumCoeff_y = 0.7154F;
            float lumCoeff_z = 0.0721F;
            float dot_res = top_x * lumCoeff_x + top_y * lumCoeff_y + top_z * lumCoeff_z;
            float res_x;
            float res_y;
            float res_z;
            if (dot_res < 0.45F) {
               res_x = 2.0F * top_x * bot_x;
               res_y = 2.0F * top_y * bot_y;
               res_z = 2.0F * top_z * bot_z;
            } else if (dot_res > 0.55F) {
               res_x = 1.0F - 2.0F * (1.0F - top_x) * (1.0F - bot_x);
               res_y = 1.0F - 2.0F * (1.0F - top_y) * (1.0F - bot_y);
               res_z = 1.0F - 2.0F * (1.0F - top_z) * (1.0F - bot_z);
            } else {
               float res1_x = 2.0F * top_x * bot_x;
               float res1_y = 2.0F * top_y * bot_y;
               float res1_z = 2.0F * top_z * bot_z;
               float res2_x = 1.0F - 2.0F * (1.0F - top_x) * (1.0F - bot_x);
               float res2_y = 1.0F - 2.0F * (1.0F - top_y) * (1.0F - bot_y);
               float res2_z = 1.0F - 2.0F * (1.0F - top_z) * (1.0F - bot_z);
               float a_tmp = (dot_res - 0.45F) * 10.0F;
               float mix_res_z = res1_z * (1.0F - a_tmp) + res2_z * a_tmp;
               float mix_res_y = res1_y * (1.0F - a_tmp) + res2_y * a_tmp;
               float mix_res_x = res1_x * (1.0F - a_tmp) + res2_x * a_tmp;
               res_x = mix_res_x;
               res_y = mix_res_y;
               res_z = mix_res_z;
            }

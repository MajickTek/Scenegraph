...
public class SWBlend_DIFFERENCEPeer extends SWEffectPeer {
   public SWBlend_DIFFERENCEPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float x_tmp_x = bot_x - top_x;
            float x_tmp_y = bot_y - top_y;
            float x_tmp_z = bot_z - top_z;
            float abs_res_z = Math.abs(x_tmp_z);
            float abs_res_y = Math.abs(x_tmp_y);
            float abs_res_x = Math.abs(x_tmp_x);

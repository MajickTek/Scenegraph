...
public class SWBlend_SOFT_LIGHTPeer extends SWEffectPeer {
   public SWBlend_SOFT_LIGHTPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float botsq_x = bot_x * bot_x;
            float botsq_y = bot_y * bot_y;
            float botsq_z = bot_z * bot_z;
            float res_x = 2.0F * bot_x * top_x + botsq_x - 2.0F * botsq_x * top_x;
            float res_y = 2.0F * bot_y * top_y + botsq_y - 2.0F * botsq_y * top_y;
            float res_z = 2.0F * bot_z * top_z + botsq_z - 2.0F * botsq_z * top_z;

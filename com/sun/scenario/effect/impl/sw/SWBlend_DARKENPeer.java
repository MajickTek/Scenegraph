...
public class SWBlend_DARKENPeer extends SWEffectPeer {
   public SWBlend_DARKENPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float min_res = bot_x < top_x ? bot_x : top_x;
            float res_x = min_res;
            min_res = bot_y < top_y ? bot_y : top_y;
            float res_y = min_res;
            min_res = bot_z < top_z ? bot_z : top_z;
            float res_w = bot_w + top_w - bot_w * top_w;
            float color_x = res_x;
            float color_y = res_y;
            float color_z = min_res;
            float color_w = res_w;
            if (res_x < 0.0F) {
               color_x = 0.0F;
            } else if (res_x > 1.0F) {

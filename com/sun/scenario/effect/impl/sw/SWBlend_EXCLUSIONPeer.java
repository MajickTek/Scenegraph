...
public class SWBlend_EXCLUSIONPeer extends SWEffectPeer {
   public SWBlend_EXCLUSIONPeer(GraphicsConfiguration gc) {
      super(gc);
   }
   ...
   public ImageData filter(Effect effect, AffineTransform transform, ImageData... inputs) {
      ...
      for(int dy = 0; dy < 0 + dsth; ++dy) {
         ...
         for(int dx = 0; dx < 0 + dstw; ++dx) {
            ...
            float res_x = top_x + bot_x - 2.0F * top_x * bot_x;
            float res_y = top_y + bot_y - 2.0F * top_y * bot_y;
            float res_z = top_z + bot_z - 2.0F * top_z * bot_z;
